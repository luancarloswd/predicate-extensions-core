...
using PredicateExtensions.UnitTests.Resources;
using Xunit;
...
namespace PredicateExtensions.UnitTests
{
    ...
    */
    ...
    public class EfExtensionTests
    {
        ...
        private Expression<Func<Post, bool>> HasTitle(string title) => post => post.Title == title;

        private Expression<Func<Post, bool>> ContentContains(string keyword) => post => post.Content.Contains(keyword);
